    ...
    if (role === 'PO') {
        ...
        WITH role_counts AS (
            ...
            WHERE 
                ...
                AND p.role_type_id IS NOT NULL  
            GROUP BY 
        ...
        expanded_permissions AS (
            ...
            FROM 
                public.active_crud_permissions_tbl a
        ),
        ...
        permission_counts AS (
            ...
            WHERE
                e.role_type IS NOT NULL  
                AND e.role_type_id IS NOT NULL  
            GROUP BY
        ...
        SELECT 
            ...
            r.role,
            SUM(r.role_count) AS role_count,  
        ...
        LEFT JOIN 
            permission_counts p
        ON 
            r.role_type_id = p.role_type_id
        GROUP BY 
            r.role, r.role_type_id, p.permission_access_count
        ORDER BY 
            r.role;        `;
    } else if (role === 'OP') {
        query = `
