...
const ProEmpforgotPassword = async (req, res) => {
  const { email_id } = req.body;
  try {
    
    const userResult = await pool.query(`SELECT * FROM pro_emp_personal_details WHERE email_id = $1`, [email_id]);
...
const ProEmpresetPassword = async (req, res) => {
  ...
  try {
    ...
    await pool.query('UPDATE pro_emp_personal_details SET password = $1 WHERE email_id = $2', [newPassword, email_id]);
...
module.exports = { ProEmpforgotPassword, ProEmpresetPassword }
