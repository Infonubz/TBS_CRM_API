...
const getOffersDealsByStatus = async (req, res) => {
    ...
    try {
      ...
      if (reqStatus == 5 && tbs_user_id == 'tbs-pro1001') {
        query = `
            SELECT *
            FROM redeem_offers
            WHERE req_status_id IN (0, 1, 2, 3, 4)
            ORDER BY GREATEST(created_date, updated_date) DESC; `;
    } else if (reqStatus == 5 && tbs_user_id.startsWith('tbs-pro_emp')) {
        query = `
            SELECT *
            FROM redeem_offers
            WHERE req_status_id IN (0, 1, 2, 3, 4, 6) AND tbs_user_id = $1
            ORDER BY GREATEST(created_date, updated_date) DESC; `;
        params = [tbs_user_id];
    } else if (reqStatus == 4) {
        query = `
        SELECT *
        FROM redeem_offers
        WHERE req_status_id IN (4)
        ${tbs_user_id.startsWith('tbs-pro1001') ? '' : 'AND tbs_user_id = $1'}
        ORDER BY GREATEST(created_date, updated_date) DESC
        `;
        params = tbs_user_id.startsWith('tbs-pro1001') ? [] : [tbs_user_id];    
      } else if (reqStatus == 3) {
        query = `
        SELECT *
        FROM redeem_offers
        WHERE req_status_id IN (3)
        ${tbs_user_id.startsWith('tbs-pro1001') ? '' : 'AND tbs_user_id = $1'}
        ORDER BY GREATEST(created_date, updated_date) DESC
        `;
        params = tbs_user_id.startsWith('tbs-pro1001') ? [] : [tbs_user_id];    
      }else if (reqStatus == 2 && tbs_user_id == 'tbs-pro1001') {
        query = `
        SELECT *
        FROM redeem_offers
        WHERE req_status_id IN (2)`
      } else if (reqStatus == 2 && tbs_user_id.startsWith('tbs-pro_emp')) {
        query = `
        SELECT *
        FROM redeem_offers
        WHERE req_status_id IN (2) AND tbs_user_id = $1`
        params = [tbs_user_id];    
      } else if (reqStatus == 1) {
        query = `
        SELECT *
        FROM redeem_offers
        WHERE req_status_id IN (6)
        ${tbs_user_id.startsWith('tbs-pro1001') ? '' : 'AND tbs_user_id = $1'}
        ORDER BY GREATEST(created_date, updated_date) DESC `;
        params = tbs_user_id.startsWith('tbs-pro1001') ? [] : [tbs_user_id];    
      } else {
        query = `
          SELECT *
          FROM redeem_offers
          WHERE req_status_id = $1 AND tbs_user_id = $2 ORDER BY GREATEST(created_date, updated_date) DESC; `;
        params = [reqStatus, tbs_user_id];
      }
...
const searchOffersDeals = async (req, res) => {
    try {
        const { req_status_id, search_term, tbs_user_id } = req.body; 
        const searchTerm = search_term ? `%${search_term.toLowerCase()}%` : null;
        ...
        if (req_status_id == 5) {
            query += `
                AND (
                    (
                        tbs_user_id LIKE 'tbs-pro_emp%' AND req_status_id IN (0, 1, 2, 3, 4, 6)
                    ) OR (
                        tbs_user_id NOT LIKE 'tbs-pro_emp%' AND req_status_id IN (0, 1, 2, 3, 4)
                    )
                )
            `;
        } else if (req_status_id == 4) {
            query += ` AND req_status_id IN (4) `;
        } else if (req_status_id == 3) {
            query += ` AND req_status_id IN (3) `;
        } else if (req_status_id == 1) {
            query += ` AND req_status_id IN (6) `;
        } else {
        ...
        if (!tbs_user_id =='tbs-pro1001') {
            query += ` AND tbs_user_id = $${paramIndex} `;
            queryParams.push(tbs_user_id);
            paramIndex++;
        }
        ...
        query += ` ORDER BY GREATEST(created_date, updated_date) DESC`;
