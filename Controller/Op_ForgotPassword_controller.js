...
const transporter = nodemailer.createTransport({
  ...
  auth: {
    ...
    pass: 'bdqbqlgqgcnnrxrr', // App password or email password
  },
});
...
const OpforgotPassword = async (req, res) => {
  const { emailid } = req.body;
  try {
    
    const userResult = await pool.query(`SELECT * FROM operators_tbl WHERE emailid = $1`, [emailid]);
...
const OpresetPassword = async (req, res) => {
  ...
  try {
    ...
    await pool.query('UPDATE operators_tbl SET password = $1 WHERE emailid = $2', [newPassword, emailid]);
...
module.exports = { OpforgotPassword, OpresetPassword }
